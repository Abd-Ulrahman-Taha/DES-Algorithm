...
public class DesAlgorithm {
    ...
    public void Encryption() {
        ...
        try {
            ...
            pw.println();
            int Index = 0;
            for (int i = 0; i < NumLeftShifts.length; i++) {
                int a = NumLeftShifts[i];
            ...
            if (plainTextLength < 64) {
                ...
                int loop;
                if (leftpadString != 0) loop = plainTextLength / 64 + 1;
                else loop = plainTextLength / 64;
                int start = 0;
                int end = 64;
                int id = loop, wordCount = 1;
                while (id >0) {
                    int leftpad = 64 - leftpadString;
                    if (leftpad % 64 != 0) leftSpace = leftpad / 8;
                    else leftSpace = 0;
                    if (plainTextLength - start < 64) end = plainTextLength;
                    else end = end;
                    ...
                    int counter = 1;
                    for (int j = 0; j < RoundKeyArray.length; j++) {
                        String k = RoundKeyArray[j];
                        pw.println();
                        ...
                        if (counter > 16) {
                            ...
                            if (id == 1 && leftSpace != 0)
                                textEncipher += intTostr(finalResult, 8).substring(leftSpace);
                            else textEncipher += intTostr(finalResult, 8);
                        }
            ...
            int loop;
            if (leftpadString != 0) loop = encipherLength / 64 + 1;
            else loop = encipherLength / 64;
            int start = 0;
            int end = 64;
            pw.println("_________________________________________________________________________________");
            for(int id = loop, wordCount = 1; id >0; id--,wordCount++) {
                int leftpad = 64 - leftpadString;
                if (encipherLength - start < 64) end = encipherLength;
                else end = end;
                ...
                for (int p = 15; p >= 0; p--) {
                    ...
                    if (counter > 16) {
                        ...
                        if (id == 1 && leftSpace != 0) decipher += intTostr(finalResult, 8).substring(leftSpace);
                        else decipher += intTostr(finalResult, 8);
